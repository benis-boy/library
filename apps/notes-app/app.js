...
function updateText(id, newText, type) {
    if (type === 'act') {
        data.acts.forEach(act => { if (act.id === id)
            act.text = newText; });
    }
    else if (type === 'chapter') {
        data.acts.forEach(act => {
            act.chapters.forEach(chapter => { if (chapter.id === id)
        ...
        data.acts.forEach(act => {
            act.chapters.forEach(chapter => {
                chapter.scenes.forEach(scene => { if (scene.id === id)
                    scene.text = newText; });
            });
        });
    }
...
function updatePointText(id, newText) {
...
window.updatePointText = updatePointText;
